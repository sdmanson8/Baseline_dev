	...
	function Complete-GuiExecutionRun
	{
		...
		if ($Mode -eq 'Defaults')
		{
			...
			if ($nextStep -eq 'Retry Safe Restore Failures')
			{
				...
				Start-GuiExecutionRun -TweakList $recoverableFailedResults -Mode 'Defaults' -ExecutionTitle (Get-UxLocalizedString -Key 'GuiExecTitleRetryingSafeRestore' -Fallback 'Retrying Safe Restore Failures')
				return
			}
			if ($nextStep -eq 'Resume Interrupted Run')
			{
				...
				Start-GuiExecutionRun -TweakList $resumeInterruptedResults -Mode 'Defaults' -ExecutionTitle (Get-UxLocalizedString -Key 'GuiExecTitleResumingInterruptedRun' -Fallback 'Resuming Interrupted Run')
				return
			}
			if ($nextStep -eq 'Open Detailed Log')
			{
		...
		try
		{
			...
			if ($Mode -eq 'Run' -and $rollbackCommandList.Count -gt 0)
			{
				try
				{
					...
					[System.IO.File]::WriteAllText($lastRunPath, ($lastRunPayload | ConvertTo-Json -Depth 12), [System.Text.UTF8Encoding]::new($false))
					$Script:LastRunProfile = [pscustomobject]$lastRunPayload
					if ((Test-GuiObjectField -Object $Script:BtnUndoLastRun -FieldName 'IsEnabled')) { $Script:BtnUndoLastRun.IsEnabled = $true }
		...
		while ($true)
		{
			...
			if ($nextStep -eq 'Retry Safe Failures')
			{
				...
				Start-GuiExecutionRun -TweakList $recoverableFailedResults -Mode 'Run' -ExecutionTitle (Get-UxLocalizedString -Key 'GuiExecTitleRetryingSafeFailures' -Fallback 'Retrying Safe Failures')
				return
			}
			if ($nextStep -eq 'Resume Interrupted Run')
			{
				...
				Start-GuiExecutionRun -TweakList $resumeInterruptedResults -Mode 'Run' -ExecutionTitle (Get-UxLocalizedString -Key 'GuiExecTitleResumingInterruptedRun' -Fallback 'Resuming Interrupted Run')
				return
			}
			if ($nextStep -eq 'Undo Game Mode Changes')
			{
				...
				Start-GuiExecutionRun -TweakList $gameModeUndoList -Mode 'Run' -ExecutionTitle (Get-UxLocalizedString -Key 'GuiExecTitleUndoingGameMode' -Fallback 'Undoing Game Mode Changes')
				return
			}
	...
	function Start-GuiExecutionRun
	{
		...
		if ($Mode -eq 'Run' -and $remoteContext -and $remoteContext.Connected -and $remoteContext.TargetComputers.Count -gt 0)
		{
			...
			finally
			{
				...
				{
					try { Remove-Item -LiteralPath $tempProfilePath -Force -ErrorAction SilentlyContinue } catch { }
				}
