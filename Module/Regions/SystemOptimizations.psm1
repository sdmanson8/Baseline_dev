...
{
	$scratchDirectory = Join-Path $env:TEMP 'Baseline'
...
function Invoke-SystemOptimizations
{
	...
	foreach ($import in $regImports)
	{
		...
		{
			$hadIssue = $true
			LogWarning "Failed to apply $($import.Description): $($_.Exception.Message)"
		}
	}
	...
	{
		$hadIssue = $true
		LogWarning "Failed to enable legacy boot menu: $($_.Exception.Message)"
	}
	...
	foreach ($environmentVariable in $environmentVariables)
	{
		...
		{
			$hadIssue = $true
			LogWarning "Failed to set environment variable $($environmentVariable.Name): $($_.Exception.Message)"
		}
	}
	...
	))
	{
		...
		{
			$hadIssue = $true
			LogWarning "Failed to update file attributes for $($command.Description): $($_.Exception.Message)"
		}
	}
	...
	{
		$hadIssue = $true
		LogWarning "Failed to remove Your Phone shortcut: $($_.Exception.Message)"
	}
