...
- 124 locale files now carry full key coverage against the English source (2,421 keys per file). A bulk repair pass corrected 176,836 strings of double-encoded UTF-8 across 121 files; `scripts/Audit-LocaleResidual.py` now reports zero mojibake remaining. Translations are machine-assisted — mechanically clean but still undergoing idiomatic QA. Cherokee was dropped as a shipped locale in this release (removed pending a formal translation partnership). See `dev_docs/locale_audit_2026-04-22.md`.
