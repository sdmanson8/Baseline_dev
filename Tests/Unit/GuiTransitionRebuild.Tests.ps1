...
Describe 'Focused GUI rebuilds' {
    ...
    It 'routes GUI region search refresh and splash-close cleanup through logged paths' {
        ...
        $script:GuiContent | Should -Match '\$Form\.ShowActivated = \$true'
        ...
        $script:GuiContent | Should -Match '\$mainWindow\.ShowActivated = \$true'
        $script:GuiContent | Should -Match '\$mainWindowInterop\.EnsureHandle\(\)'
        $script:GuiContent | Should -Match '\[WinAPI\.ForegroundWindow\]::ShowWindowAsync\(\$mainWindowHandle, 5\)'
        $script:GuiContent | Should -Match '\[WinAPI\.ForegroundWindow\]::ShowWindowAsync\(\$mainWindowHandle, 9\)'
        $script:GuiContent | Should -Match '\$hwndTopMost = \[IntPtr\]::new\(-1\)'
        $script:GuiContent | Should -Match '\[WinAPI\.ForegroundWindow\]::SetWindowPos\(\$mainWindowHandle, \$hwndTopMost, 0, 0, 0, 0, 0x43\)'
        $script:GuiContent | Should -Match '\[WinAPI\.ForegroundWindow\]::SetForegroundWindow\(\$mainWindowHandle\)'
        $script:GuiContent | Should -Match '\$mainWindow\.Topmost = \$true'
        $script:GuiContent | Should -Match 'Start-Sleep -Milliseconds 900'
        $script:GuiContent | Should -Match '\$mainWindow\.Topmost = \$false'
        $script:GuiContent | Should -Match '\$hwndNoTopMost = \[IntPtr\]::new\(-2\)'
        $script:GuiContent | Should -Match '\[WinAPI\.ForegroundWindow\]::SetWindowPos\(\$mainWindowHandle, \$hwndNoTopMost, 0, 0, 0, 0, 0x43\)'
        $script:GuiContent | Should -Match '\$null = \$mainWindow\.Activate\(\)'
        $script:GuiContent | Should -Match '\$null = \$mainWindow\.Focus\(\)'
        $script:GuiContent | Should -Match 'SplashClose runspace: mainWindow presentation transition failed'
        $script:GuiContent | Should -Match 'SplashClose runspace: mainWindow activated after splash close'
